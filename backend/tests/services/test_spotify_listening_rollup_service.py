...
def test_refresh_builds_queryable_rollups_and_collection_matches() -> None:
    with _db_session() as db:
        ...
        result = SpotifyListeningRollupService().refresh(db)
        ...
        top_artist = repository.list_top_artists(db, limit=1)[0]
        ...
        top_album = repository.list_top_albums(db, limit=1)[0]
        assert top_album.album_name == "Music Has the Right to Children"
        assert top_album.play_count == 2

        hourly = {row.played_hour: row.play_count for row in repository.list_hourly_stats(db)}
        assert hourly == {1: 1, 4: 1, 5: 1}

        monthly = repository.list_monthly_artist_stats(db, normalized_artist_name="boards of canada")
        assert len(monthly) == 1
        assert monthly[0].played_year_month == "2019-02"
        assert monthly[0].play_count == 2

        skipped = [row for row in repository.list_skip_stats(db) if row.skipped is True]
        assert len(skipped) == 1
        assert skipped[0].reason_end == "fwdbtn"

        artist_match = db.get(SpotifyVinylArtistMatch, "boards of canada")
        ...
        release_match = repository.list_release_matches(db, limit=1)[0]
